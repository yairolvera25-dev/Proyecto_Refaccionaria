...
import { authService } from '../services/authService'; // Ajusté la ruta
import FondoMeteoritos from '../components/FondoMeteoritos.vue'; // Ajusté la ruta
...
const tema = computed(() => {
  switch (rolActivo.value) {
    case 'administrador': return { texto: 'text-[#ef4444]', borde: 'border-[#ef4444] shadow-[0_0_40px_rgba(239,68,68,0.2)]', inputFocus: 'focus:border-[#ef4444]', boton: 'bg-[#ef4444] hover:bg-[#dc2626]' };
    case 'vendedor': return { texto: 'text-[#22c55e]', borde: 'border-[#22c55e] shadow-[0_0_40px_rgba(34,197,94,0.2)]', inputFocus: 'focus:border-[#22c55e]', boton: 'bg-[#22c55e] hover:bg-[#16a34a]' };
    case 'consultor': return { texto: 'text-[#3b82f6]', borde: 'border-[#3b82f6] shadow-[0_0_40px_rgba(59,130,246,0.2)]', inputFocus: 'focus:border-[#3b82f6]', boton: 'bg-[#3b82f6] hover:bg-[#2563eb]' };
    default: return { texto: 'text-gray-500', borde: 'border-gray-800', inputFocus: 'focus:border-gray-500', boton: 'bg-gray-800' };
  }
});
...
const iniciarSesion = async () => {
  ...
  cargando.value = true;
  try {
    const data = await authService.login(credenciales.value.email, credenciales.value.password);
    const userRole = data.user.role.toLowerCase();
    
    if (userRole !== rolActivo.value) {
      throw new Error(`Tu cuenta es de ${userRole}, no de ${rolActivo.value}`);
    }

    localStorage.setItem('token', data.token);
    localStorage.setItem('user', JSON.stringify(data.user));

    mensaje.value = { texto: `¡Acceso concedido, ${data.user.nombre}!`, tipo: 'exito' };

    setTimeout(() => {
      if (userRole === 'administrador') router.push('/admin');
      else if (userRole === 'vendedor') router.push('/vendedor');
      else if (userRole === 'consultor') router.push('/consultor');
    }, 1000);
  } catch (error) {
    mensaje.value = { texto: error.message || 'Error de conexión', tipo: 'error' };
  } finally {
  ...
  <div class="fixed inset-0 bg-[#080d17] text-white flex flex-col font-sans overflow-hidden">
    ...
    <div class="flex-1 w-full max-w-[1400px] mx-auto flex flex-col px-6 md:px-12 py-10 z-10 overflow-y-auto">
      ...
      <div class="w-full max-w-3xl mx-auto flex flex-col items-center">
        ...
        <div v-if="mensaje.texto" :class="['w-full p-4 rounded-lg text-sm text-center font-bold mb-6 tracking-wider backdrop-blur-md', mensaje.tipo === 'error' ? 'bg-red-500/10 text-red-400 border border-red-500/30' : 'bg-green-500/10 text-green-400 border border-green-500/30']">
          {{ mensaje.texto }}
        </div>
        <form @submit.prevent="iniciarSesion" class="w-full space-y-6">
          <input v-model="credenciales.email" type="email" placeholder="Email" required :class="['w-full bg-[#0d1424]/80 backdrop-blur-sm border-b-2 border-gray-800 rounded-t-lg px-6 py-5 text-white placeholder-gray-600 outline-none transition-all duration-300 text-base md:text-lg', tema.inputFocus]">
          <input v-model="credenciales.password" type="password" placeholder="Password" required :class="['w-full bg-[#0d1424]/80 backdrop-blur-sm border-b-2 border-gray-800 rounded-t-lg px-6 py-5 text-white placeholder-gray-600 outline-none transition-all duration-300 text-base md:text-lg', tema.inputFocus]">
